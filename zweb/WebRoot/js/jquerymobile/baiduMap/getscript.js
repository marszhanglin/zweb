...
(function() {
		...
		if (!value) {
			var fixer = dom._styleFixer[key], style = element.currentStyle
					|| (baidu.browser.ie ? element[Tstyle] : getComputedStyle(
							element, null));
			value = fixer && fixer.get ? fixer.get(element, style)
					: style[fixer || key]
		}
	...
	baidu.enableTapEvent = function() {
		...
		Tap[Tprototype] = {
			...
			move : function(e) {
				stopAndPrevent(e);
				var x = this.hasTouch ? e.touches[0].clientX : e.clientX, y = this.hasTouch ? e.touches[0].clientY
						: e.clientY;
					...
					Map[Tprototype],
					{
						...
						_bind : function() {
							...
							me._watchSize = function() {
								var wh = me.getSize();
								if (me[Twidth] != wh[Twidth]
										|| me[Theight] != wh[Theight]) {
									var originSize = new Size(me[Twidth],
											me[Theight]);
									var oe = new BaseEvent("onbeforeresize");
									oe.size = originSize;
									me.dispatchEvent(oe);
									me._updateCenterPoint(
											(wh[Twidth] - me[Twidth]) / 2,
							...
							if (me.config.enableAutoResize)
								me.temp.autoResizeTimer = setInterval(
										me._watchSize, 80)
						},
						_updateCenterPoint : function(xn, yn, cp, useCp) {
							var zu = this.getMapType().getZoomUnits(
									this.getZoom());
							var pro = this.projection;
							var reAssign = true;
							if (cp && Point.isInRange(cp)) {
								this.centerPoint = new Point(cp["lng"],
										cp["lat"]);
								reAssign = false
							}
							var mcp = cp && useCp ? pro.lngLatToMercator(cp,
									this.currentCity) : this.mercatorCenter;
							if (mcp) {
								this.mercatorCenter = new Point(mcp["lng"] + xn
										* zu, mcp["lat"] - yn * zu);
								var p = pro.mercatorToLngLat(
						...
						zoomTo : function(level, point) {
							...
							if (cPoint) {
								var cPixel = this.pointToPixel(cPoint,
										this.lastLevel);
								this._updateCenterPoint(this[Twidth] / 2
										- cPixel.x, this[Theight] / 2
										- cPixel.y, this.pixelToPoint(cPixel,
										this.lastLevel), true)
							}
							this.dispatchEvent(new BaseEvent("onzoomstart"));
							this
									.dispatchEvent(new BaseEvent(
											"onzoomstartcode"))
						},
						...
						panTo : function(center, opts) {
							...
							this.mercatorCenter = this.projection
									.lngLatToMercator(center, this.currentCity);
							if (Point.isInRange(center))
								this.centerPoint = new Point(center["lng"],
										center["lat"]);
							else
						...
						addTileLayer : function(tilelayer) {
							if (tilelayer)
								this.dispatchEvent(new BaseEvent(
										"onaddtilelayer", tilelayer))
						},
						removeTileLayer : function(tilelayer) {
							if (tilelayer)
								this.dispatchEvent(new BaseEvent(
										"onremovetilelayer", tilelayer))
						},
						...
						centerAndZoom : function(center, zoom) {
							...
							me.zoomLevel = zoom;
							me.centerPoint = new Point(center["lng"],
									center["lat"]);
							me.mercatorCenter = me.projection.lngLatToMercator(
									me.centerPoint, me.currentCity);
							me.defaultZoomLevel = me.defaultZoomLevel
									|| me.zoomLevel;
							me.defaultCenter = me.defaultCenter
									|| me.centerPoint;
							var e = new BaseEvent("onload");
							var ecode = new BaseEvent("onloadcode");
							e["point"] = new Point(center["lng"], center["lat"]);
							e["pixel"] = me.pointToPixel(me.centerPoint,
									me.zoomLevel);
							e["zoom"] = zoom;
							if (!me.loaded) {
								me.loaded = true;
						...
						getSize : function() {
							if (this.customSize
									&& this.customSize instanceof Size)
						...
						setSize : function(size) {
							if (size && size instanceof Size) {
								...
								this.container.style[Twidth] = size[Twidth]
										+ "px";
								this.container.style[Theight] = size[Theight]
										+ "px"
							} else
						...
						"pointToPixelFor3D" : function(point, callback) {
							...
							if (mapType == BMAP_PERSPECTIVE_MAP && curCity)
								CoordTransUtils.convertPtToPix3D(point, me,
										callback)
						},
						pixelToPointFor3D : function(pixel, callback) {
							...
							var mapType = me.mapType, curCity = map.currentCity;
							if (mapType == BMAP_PERSPECTIVE_MAP && curCity)
								CoordTransUtils.convertPixToPt3D(pixel, me,
										callback)
						},
						...
						getBounds : function() {
							...
							var opts = arguments[0] || {}, margins = opts["margins"]
									|| [ 0, 0, 0, 0 ], zoom = opts["zoom"]
									|| null, pt1 = this.pixelToPoint({
								x : margins[3],
						...
						_getBestLevel : function(bounds, opts) {
							...
							var margins = opts["margins"] || [ 10, 10, 10, 10 ], zoomFactor = opts["zoomFactor"] || 0, ew = margins[1]
									+ margins[3], eh = margins[0] + margins[2], minLevel = mapType
									.getMinZoom(), maxLevel = mapType
									.getMaxZoom();
							for (var level = maxLevel; level >= minLevel; level--) {
								var zoomUnits = this.getMapType().getZoomUnits(
										level);
								if (bounds.toSpan()["lng"] / zoomUnits < this[Twidth]
										- ew
						...
						getViewport : function(view, opts) {
							...
							if (!view || !view instanceof Bounds
									&& view[Tlength] == 0
							...
							for (var i = 0, l = points[Tlength]; i < l; i++)
								mercatorPts.push(this.projection
										.lngLatToMercator(points[i],
												this.currentCity));
							...
							if (opts["margins"]) {
								var margins = opts["margins"], wBias = (margins[1] - margins[3]) / 2, hBias = (margins[0] - margins[2]) / 2, zoomUnit = this
										.getMapType().getZoomUnits(zoom);
								if (opts["offset"]) {
									wBias = opts["offset"][Twidth];
									hBias = opts["offset"][Theight]
								}
								center["lng"] = center["lng"] + zoomUnit
										* wBias;
								center["lat"] = center["lat"] + zoomUnit
										* hBias
							}
							center = this.projection.mercatorToLngLat(center,
									this.currentCity);
							return {
								"center" : center,
								"zoom" : zoom
						...
						setViewport : function(view, opts) {
							...
							if (viewport.zoom == this.zoomLevel
								...
								this.centerAndZoom(viewport["center"],
										viewport["zoom"])
						},
						getPanes : function() {
							return this._panes
						...
						"getDistance" : function(start, end) {
							if (!start || !end)
								return;
							var dis = 0;
							dis = MercatorProjection
									.getDistanceByLL(start, end);
							return dis
						},
						...
						setCurrentCity : function(city) {
							var me = this;
							me.currentCity = BMAP_PERSPECTIVE_MAP
									.getCityName(city);
							me.cityCode = BMAP_PERSPECTIVE_MAP
									.getCityCode(me.currentCity);
							if (me.mapType == BMAP_PERSPECTIVE_MAP)
						...
						_checkZoom : function() {
							var mapTypeMinZoom = this.highResolutionEnabled() ? this.mapType._opts.highMinZoom
									: this.mapType.getMinZoom();
							var mapTypeMaxZoom = this.highResolutionEnabled() ? this.mapType._opts.highMaxZoom
									: this.mapType.getMaxZoom();
							var config = this.config;
							config.minZoom = config.userMinZoom
									|| mapTypeMinZoom;
							config.maxZoom = config.userMaxZoom
									|| mapTypeMaxZoom;
	...
	function _addStat(code, opts) {
		...
		if (_addStat._sending)
			_addStat._reqQueue.push({
				src : "product=jsapi&v=" + BMap.version + "&t=" + ts + "&code="
						+ code + extq
			});
		else
			sendStat({
	...
	var DataMgr = {
		request : function(url) {
			...
			setTimeout(
					function() {
						Tdocument.getElementsByTagName("head")[0][TappendChild]
								(script);
						script = null
					}, 1)
		}
										...
										setTimeout(
												function() {
													...
													if (mdlsFromServer[Tlength] == 0)
														...
														var modpath = me.Config._baseUrl
																+ "&mod="
																+ mdlsFromServer
																		.join(",");
														DataMgr
					...
					Control[Tprototype],
					{
						...
						remove : function() {
							...
							this._container[TparentNode]
									&& this._container[TparentNode][TremoveChild]
											(this._container);
							this._container._jsobj = null;
							this._container = null
						},
						_render : function() {
							this._container = beforeEndHTML(
									this._map.container,
						...
						setAnchor : function(anchor) {
							if (this.anchorFixed || !isNumber(anchor)
									|| isNaN(anchor)
							...
							var a = [ "TL", "TR", "BL", "BR" ];
							baidu.dom.removeClass(this._container, "anchor"
									+ a[preAnchor]);
							baidu.dom.addClass(this._container, "anchor"
									+ a[anchor])
						},
						getAnchor : function() {
						...
						setOffset : function(size) {
							...
							this._opts.offset = new Size(size[Twidth],
									size[Theight]);
							if (!this._container)
								return;
							this.setAnchor(this._opts.anchor)
	...
	function NavigationControl(opts) {
		...
		this["defaultAnchor"] = isMobile() ? BMAP_ANCHOR_BOTTOM_RIGHT
	...
	baidu.object[Textend](ScaleControl[Tprototype], {
		...
		setUnit : function(unit) {
			this._opts.unit = this._units[unit] && this._units[unit].name
					|| this._opts.unit
		},
		getUnit : function() {
	...
	baidu.object[Textend](Bounds[Tprototype], {
		...
		containsBounds : function(bounds) {
			if (!(bounds instanceof Bounds) || this.isEmpty()
					|| bounds.isEmpty())
				return false;
			return bounds._swLng > this._swLng && bounds._neLng < this._neLng
		...
		intersects : function(bounds) {
			...
			if (TMath.max(bounds._swLng, bounds._neLng) < TMath.min(
					...
					|| TMath.min(bounds._swLng, bounds._neLng) > TMath.max(
							this._swLng, this._neLng)
					|| TMath.max(bounds._swLat, bounds._neLat) < TMath.min(
							this._swLat, this._neLat)
					|| TMath.min(bounds._swLat, bounds._neLat) > TMath.max(
							this._swLat, this._neLat))
				return null;
			...
			return new Bounds(new Point(newMinX, newMinY), new Point(newMaxX,
					newMaxY))
		},
		containsPoint : function(point) {
			if (!(point instanceof Point) || this.isEmpty())
				return false;
			return point["lng"] >= this._swLng && point["lng"] <= this._neLng
		...
		toSpan : function() {
			...
			return new Point(TMath.abs(this._neLng - this._swLng), TMath
	...
	baidu[Textend](MercatorProjection, {
		...
		MC2LL : [
				...
				[ -3.030883460898826E-8, 8.98305509983578E-6, 0.30071316287616,
						...
						6856817.37 ],
				[ -1.981981304930552E-8, 8.983055099779535E-6,
	...
	baidu[Textend](MercatorProjection[Tprototype], {
		...
		pointToPixel : function(point, zoom, mapCenter, mapSize, curCity) {
			...
			var x = TMath.round((point["lng"] - mapCenter["lng"]) / zoomUnits
					+ mapSize[Twidth] / 2);
			var y = TMath.round((mapCenter["lat"] - point["lat"]) / zoomUnits
					+ mapSize[Theight] / 2);
	...
	baidu[Textend]
			(
					PerspectiveProjection[Tprototype],
					{
						lngLatToMercator : function(lngLat, mapCity) {
							return this._convert2DTo3D(mapCity,
									MercatorProjection.convertLL2MC(lngLat))
						},
						mercatorToLngLat : function(mercator, mapCity) {
							return MercatorProjection.convertMC2LL(this
									._convert3DTo2D(mapCity, mercator))
						},
						"lngLatToPointFor3D" : function(lngLat, callback) {
							...
							ModuleMgr.load("coordtrans", function() {
								var obj = CoordTrans.getOMap_pts(me.curCityName
										|| "bj", mcPt);
						...
						"pointToLngLatFor3D" : function(point, callback) {
							...
							ModuleMgr.load("coordtrans", function() {
								var obj = CoordTrans.getMapJw_pts(
										me.curCityName || "bj", pixPt);
								var mcPt = new Point(obj["lng"], obj["lat"]);
								var llPt = MercatorProjection
										.convertMC2LL(mcPt);
						...
						_convert2DTo3D : function(city, point) {
							if (ModuleMgr.current("coordtrans")._status == ModuleMgr.Request.COMPLETED) {
								var obj = CoordTrans.getOMap_pts(city || "bj",
										point);
						...
						_convert3DTo2D : function(city, point) {
							if (ModuleMgr.current("coordtrans")._status == ModuleMgr.Request.COMPLETED) {
								var obj = CoordTrans.getMapJw_pts(city || "bj",
										point);
			...
			(
					Division[Tprototype],
					{
						_addDom : function() {
							...
							if (config.borderColor != "none")
								html.push("border:" + config.lineStroke + "px "
										+ config.lineStyle + " "
										+ config.lineColor + ";");
							html.push("opacity:" + config.opacity
									+ "; filter:(opacity=" + config.opacity
									* 100 + ")");
							...
							this.domElement = beforeEndHTML(
									this.map.getPanes().markerMouseTarget, html
											.join(""))
						},
						"initialize" : function(map) {
							this.map = map;
							this._addDom();
							if (this.domElement) {
								var eventName = isMobile() ? "touchstart"
										: "mousedown";
						...
						"draw" : function() {
							...
							this._config.offset = new Size(
									-TMath.round(this._config[Twidth] / 2)
											- TMath
													.round(this._config.lineStroke),
									-TMath.round(this._config[Theight] / 2)
											- TMath
													.round(this._config.lineStroke));
							this.domElement.style.left = pix.x
									+ this._config.offset[Twidth] + "px";
							this.domElement.style.top = pix.y
									+ this._config.offset[Theight] + "px"
						},
						getPosition : function() {
						...
						setDimension : function(w, h) {
							...
							if (this.domElement) {
								this.domElement.style[Twidth] = this._config[Twidth]
										+ "px";
								this.domElement.style[Theight] = this._config[Theight]
										+ "px";
								this["draw"]()
							}
						}
	...
	function InfoWindow(content, opts) {
		...
		this._config = {
			...
			enableCloseOnClick : opts["enableCloseOnClick"] === false ? false
					: true,
			margin : [ 10, 10, 40, 10 ],
	...
	baidu[Textend](Label[Tprototype], {
		...
		setPosition : function(point) {
			if (point instanceof Point && !this.getMarker())
				this.point = this._config[Tposition] = new Point(point["lng"],
						point["lat"])
		},
		setContent : function(content) {
			this.content = content
	...
	function Polygon(points, opts) {
		...
		opts = opts || {};
		this._config.fillOpacity = opts["fillOpacity"] ? opts["fillOpacity"]
				: 0.65;
		if (opts.fillColor == "")
			this._config.fillColor = "";
		else
			this._config.fillColor = opts["fillColor"] ? opts["fillColor"]
					: "#fff";
		this.setPath(points);
		var me = this;
		ModuleMgr.load("poly", function() {
	...
	baidu[Textend](Polygon[Tprototype], {
		...
		setPositionAt : function(index, point) {
			...
			if (index == 0
					&& !this.points[0]
							.equals(this.points[this.points[Tlength] - 1]))
				this.points[this.points[Tlength] - 1] = new Point(point["lng"],
						point["lat"]);
			this._calcBounds()
		},
		getPath : function() {
			...
			(
					Circle[Tprototype],
					{
						...
						_getPerimeterPoints : function(center, radius) {
							...
							var points = [];
							var d = radius / 6378800, lat1 = TMath.PI / 180
									* cy, lng1 = TMath.PI / 180 * cx;
							for (var i = 0; i < 360; i += 9) {
								var tc = TMath.PI / 180 * i, y = TMath
										.asin(TMath.sin(lat1) * TMath.cos(d)
												+ TMath.cos(lat1)
												* TMath.sin(d) * TMath.cos(tc)), dlng = TMath
										.atan2(TMath.sin(tc) * TMath.sin(d)
												* TMath.cos(lat1), TMath.cos(d)
												- TMath.sin(lat1)
												* TMath.sin(y)), x = (lng1
										- dlng + TMath.PI)
										% (2 * TMath.PI) - TMath.PI, point = new Point(
										x * (180 / TMath.PI), y
												* (180 / TMath.PI));
								points.push(point)
							}
							var fstPoint = points[0];
							points.push(new Point(fstPoint["lng"],
									fstPoint["lat"]));
							return points
						}
					});
			...
			(
					TileMgr[Tprototype],
					{
						initialize : function() {
							...
							map[TaddEventListener]("removetilelayer", function(
									e) {
						...
						showTile : function(tileInfo, centerPos, tilelayer) {
							...
							if (mapType === BMAP_PERSPECTIVE_MAP
									&& me.map.getZoom() == 15)
								rpix = 0.5;
							var posY = (rpix - 1 - tileInfo[1]) * tileSize
									+ centerPos[1];
							...
							var zoomUnits = 256 * TMath.pow(2, mapType
									.getMaxZoom()
									- tileInfo[2]);
							var point = new Point(tileInfo[0] * zoomUnits,
									tileInfo[1] * zoomUnits);
							var tileCoord = new Pixel(tileInfo[0], tileInfo[1]);
							var src = tilelayer["getTilesUrl"](tileCoord,
									tileInfo[2]);
							tile = new Tile(this, src, position, tileInfo,
									tilelayer);
							tile._addLoadCbk(function() {
								me._checkTilesLoaded()
							});
						...
						_checkTilesLoaded : function() {
							...
							if (this.numLoading == 0) {
								...
								this._checkLoadedTimer = setTimeout(
										function() {
											if (me.numLoading == 0) {
												me.map
														.dispatchEvent(new BaseEvent(
						...
						getTileName : function(tileInfo, tilelayer) {
							if (this.map.getMapType() === BMAP_PERSPECTIVE_MAP)
								return "TILE-" + tilelayer.guid + "-"
										+ this.map.cityCode + "-" + tileInfo[0]
										+ "-" + tileInfo[1] + "-" + tileInfo[2];
							else
								return "TILE-" + tilelayer.guid + "-"
										+ tileInfo[0] + "-" + tileInfo[1] + "-"
										+ tileInfo[2]
						},
						...
						moveGridTiles : function() {
							var me = this, map = me.map, mapType = map
									.getMapType();
							if (mapType == BMAP_PERSPECTIVE_MAP)
								ModuleMgr.load("coordtrans", function() {
									me._moveGridTiles()
						...
						_moveGridTiles : function() {
							...
							for (var n = 0; n < layernums; n++) {
								...
								var levelUnits = mapType
										.getZoomFactor(zoomLevel);
								var row = TMath.ceil(centerPoint["lng"]
										/ levelUnits);
								var column = TMath.ceil(centerPoint["lat"]
										/ levelUnits);
								var tileSize = mapType.getTileSize();
								var cell = [
										row,
										column,
										(centerPoint["lng"] - row * levelUnits)
												/ levelUnits * tileSize,
										(centerPoint["lat"] - column
												* levelUnits)
												/ levelUnits * tileSize ];
								var fromRow = cell[0]
										- TMath
												.ceil((map[Twidth] / 2 - cell[2])
														/ tileSize);
								var fromColumn = cell[1]
										- TMath
												.ceil((map[Theight] / 2 - cell[3])
														/ tileSize);
								var toRow = cell[0]
										+ TMath
												.ceil((map[Twidth] / 2 + cell[2])
								...
								var centerPixX = -this.areaCenter["lng"]
										/ zoomUnits;
								var centerPixY = this.areaCenter["lat"]
										/ zoomUnits;
								...
								tilelayer.tilesDiv.style.left = TMath
										...
										- centerPosition[0] + "px";
								tilelayer.tilesDiv.style.top = TMath
										.ceil(centerPixY + divTop)
										- centerPosition[1] + "px";
								var tilesOrder = [];
						...
						addTileLayer : function(e) {
							...
							for (var i = 0; i < me.tileLayers[Tlength]; i++)
								if (me.tileLayers[i] == tilelayer)
									return;
							tilelayer.initialize(this.map,
									this._normalLayerContainer);
	...
	baidu[Textend](TileLayer[Tprototype], {
		initialize : function(map, container) {
			...
			if (!this.tilesDiv) {
				...
				style.left = TMath.ceil(-map[ToffsetX] + map[Twidth] / 2)
						+ "px";
				style.top = TMath.ceil(-map[ToffsetY] + map[Theight] / 2)
						+ "px";
				container[TappendChild](div);
				this.tilesDiv = div
			}
	...
	var BMAP_HYBRID_MAP = new MapType(
			"\u6df7\u5408",
			[ satelliteTileLayer, streetLayer ],
			{
				"tips" : "\u663e\u793a\u5e26\u6709\u8857\u9053\u7684\u536b\u661f\u5f71\u50cf",
				"labelText" : "\u8def\u7f51",
				"minZoom" : 1,
					...
					LocalSearch[Tprototype],
					{
						...
						searchNearby : function(keyword, center, radius,
							...
							this._queryList
									.push({
										method : "searchNearby",
										arguments : [ keyword, center, radius,
												options ]
									})
						},
						...
						setPageCapacity : function(cap) {
							if (typeof cap == "number" && !isNaN(cap))
								this._opts.pageCapacity = cap < 1 ? LocalSearch.DEFAULT_PAGE_CAPACITY
										: cap > LocalSearch.MAX_PAGE_CAPACITY ? LocalSearch.DEFAULT_PAGE_CAPACITY
												: cap;
							else
	...
	TransitRoute.LINE_TYPE_MAPPING = [ 0, 1, 0, 0, 0, 0, 0, 0, 2, 0, 0, 0, 1,
			1, 1 ];
