...
import z.pn.xmpp.router.PacketRouter;
...
public class StanzaHandler {
    ...
    public void process(String stanza, XMPPPacketReader reader)
            throws Exception {
        boolean initialStream = stanza.startsWith("<stream:stream"); //会话开始
        ...
        if (stanza.equals("</stream:stream>")) {  //会话结束关闭session
            session.close();
            return;
        }
        ...
        } else if ("message".equals(tag)) {//处理聊天消息内容
            processMessage(doc);
        } else if ("presence".equals(tag)) {//处理客户端在线状态
            log.debug("presence...");
            processPresence(doc);
        } else if ("iq".equals(tag)) {//处理注册消息
            log.debug("iq...");
            processIQ(doc);
        } else {
    ...
    private void createSession(XmlPullParser xpp)
        ...
        if ("jabber:client".equals(namespace)) {//命名空间是否为  jabber:client
